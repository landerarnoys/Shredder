...
namespace Howest.NMCT.Shredder.StoreApp
{
    ...
    public sealed partial class DetailPhotoPage : Page
    {
        ...
        {
            int oldrating = _vm.ActivePicture.Rating;
            int r = Convert.ToInt32(ratPicture.Value);
            double som = oldrating + r;
            double newrating = Math.Ceiling(som / 2);
            _vm.ActivePicture.Rating = Convert.ToInt32(newrating);
            _vm.UpdatePictureRating(_vm.ActivePicture);
            ratPicture.Value = _vm.ActivePicture.Rating;
        }
